...
Используй node-id=794:24945 как единственный источник структуры UI.  
Получение данных из Figma выполнять через Figma MCP tool (node data).  
ответ MCP должен быть "design_context": { "framework": "jetpack_compose"},  
"language": "kotlin", а не "design_context": {  
  "framework": "react",  
  "language": "typescript"} и прочее  
 При вызове инструмента нужно всегда передавать:
